...
export const providers: Record<Provider,string> = { codex: 'Codex', claude: 'Claude Code', ollama: 'Ollama', mock: '모의 실행', open_ai: 'OpenAI 호환 API', command: '실행 명령' };
...
export const stateLabel = (run: {state:RunState;phase:string}) => run.state==='running'&&['생각 중','답변 작성 중'].includes(run.phase) ? run.phase : states[run.state];
...
export const providerName = (run: {provider:Provider;provider_id?:string|null;runtime?:{provider_name?:string|null}}, connections: {id:string;name:string}[] = []) => connections.find(p=>p.id===run.provider_id)?.name ?? run.runtime?.provider_name ?? providers[run.provider];
