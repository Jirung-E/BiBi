...
const checkSignature=$derived(JSON.stringify([editing?.id,editing?.host_id,editing?.adapter,editing?.command,editing?.endpoint,argsText,apiKey,clearKey]));
...
function edit(p?:ProviderConfig){resetCheck();isNew=!p;editing=p?{...p,args:[...p.args],models:[...p.models]}:{id:submissionId().replace('submission_','provider_'),name:'',host_id:'local',remote_id:null,adapter:'' as Provider,command:'',args:[],endpoint:'',models:[],api_key_set:false};modelText=p?.models.join('\n')??'';argsText=p?JSON.stringify(p.args):'[]';apiKey='';clearKey=false;error='';}
function template(value:ProviderTemplate){if(!editing)return;resetCheck();editing={...editing,...value,args:[],models:[],api_key_set:false};argsText='[]';modelText='';apiKey='';clearKey=false;error='';}
function draft(){
