...
import {age,providers,states,shortId} from '../format';
let {run,work,host,now,onopen,onclose}: {run:Run;work:Work|undefined;host:Host|undefined;now:number;onopen:()=>void;onclose:()=>void} = $props();
...
<aside class="run-details card">
 ...
 <div><small>{run.role} · {providers[run.provider]}</small><h2>{run.title}</h2></div>
