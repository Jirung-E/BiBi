...
<div class="session-actions">
 ...
 {#if edit}<form class="inline-confirm" onsubmit={(e)=>{e.preventDefault();void save({type:'rename_session',run_id:run.id,title});}}><input aria-label="새 세션 이름" bind:value={title} required maxlength="120" /><button class="primary" disabled={busy}>저장</button><button type="button" onclick={()=>edit=false}>취소</button></form>{/if}
 {#if removing}<div class="inline-confirm"><span>목록에서 제거합니다. 설정에서 복원할 수 있습니다.</span><button class="danger-button" disabled={busy} onclick={()=>save({type:'set_session_hidden',run_id:run.id,hidden:true})}>제거 확인</button><button onclick={()=>removing=false}>취소</button></div>{/if}
